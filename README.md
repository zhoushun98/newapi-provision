...
- `seed.json` — 权威配置数据（从线上实例导出，可按需修改）：
  - 14 个模型的元信息：名称、供应商归属、图标、标签、简介、端点、匹配类型
...
- **供应商不由脚本管理**——脚本不创建、不修改供应商，只按名称查询并把模型绑定过去。全新系统的供应商列表是空的（连 OpenAI 都没有），先在 UI 建好 seed 用到的供应商（OpenAI、Anthropic、DeepSeek、智谱、xAI）再跑脚本；顺序反了也没关系——模型会先以未绑定状态创建，**建好供应商后重跑一遍，脚本会自动补绑定**。
