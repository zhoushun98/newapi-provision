...
- **改一个已有模型的价，四张表要一起改**：厂商降价时 `ModelRatio` / `CompletionRatio` / `CacheRatio` 和 `billing_expr` 里的绝对价必须同步，只改一处会让分档与兜底倍率打架。`gpt-5.6-terra` 跟进降价（$2.5/$15 → $2/$12）时就是整组同步改的。

- **`gpt-5.6-sol` 与 `gpt-5.6-luna` 是有意偏离官方价**，按决策保留旧价，这不是过期数据，别当成漏更新顺手「修正」。`gpt-5.6-terra` 是这一组里唯一跟进官方降价的：

  | 模型 | 本仓库沿用（短档 / 长档） | 官方现价（短档 / 长档） | 要跟进时倍率改成 |
  |---|---|---|---|
  | `gpt-5.6-sol` | $5/$30 / $10/$45 | $4/$20 / $8/$30 | `2 / 5 / 0.1` |
  | `gpt-5.6-luna` | $1/$6 / $2/$9 | $0.2/$1.2 / $0.4/$1.8 | `0.1 / 6 / 0.1` |

  跟进时记得 `billing_expr` 的绝对价（含 `cc` 缓存写入档）要与倍率一起换。

- **`codex-auto-review` 没有官方定价可核**，是 Codex 内部的路由别名而非公开模型（[openai/codex#20981](https://github.com/openai/codex/issues/20981) 问「它到底是内部别名、公开标签还是计费身份」，至今无官方回复）。本仓库按决策让它与 `gpt-5.6-sol` 同价（$5/$30，长档 $10/$45），**这是自定价，不是抄来的官方价**——改它时不必去找官方表，跟着 sol 走即可。注意实际成本取决于渠道把它转发到哪个真实模型，而渠道不在种子范围内。
